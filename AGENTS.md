...
npm run test
npm run build
npm run team:order # regenerate member-order.json (run after adding/removing members)
```

Within-group member order is `startDate` ascending, then by **PR submission order** (earliest-added first) via `src/lib/content/member-order.json` — a committed map regenerated by `npm run team:order` (git add-order). After merging a member PR, run it and commit the updated JSON. `order` frontmatter still overrides when set.
