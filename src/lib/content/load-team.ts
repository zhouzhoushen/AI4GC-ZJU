...
// Member folder -> repo add order (≈ PR submission order), regenerated by
// `npm run team:order`. Used as the within-group tiebreaker after start date.
import memberOrderJson from "@/lib/content/member-order.json";
...
function sortMemberEntries(entries: { folder: string; member: TeamMember }[]): TeamMember[] {
  return [...entries]
    .sort((a, b) => {
      ...
      // Explicit `order` frontmatter wins when both members set it.
      const orderA = a.member.order;
      const orderB = b.member.order;
      if (orderA != null && orderB != null && orderA !== orderB) return orderA - orderB;
      if (orderA != null && orderB == null) return -1;
      if (orderA == null && orderB != null) return 1;

      // Otherwise the member added earliest (≈ first PR submitted) comes first.
      const addA = MEMBER_ADD_ORDER[a.folder] ?? Number.MAX_SAFE_INTEGER;
      const addB = MEMBER_ADD_ORDER[b.folder] ?? Number.MAX_SAFE_INTEGER;
...
function loadMembersFromGroup(group: TeamContentGroup): TeamMember[] {
  ...
  const entries = listMemberFolders(groupDir).map((folder) => ({
    folder,
    member: loadMemberFile(group, folder, "member").member,
  }));

  return sortMemberEntries(entries);
}
